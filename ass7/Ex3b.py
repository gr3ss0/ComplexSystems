...
for i in range(100):
	...
	states, times = SSA(S,X0,t_final,k, timed=True)
	print("Trajectory ", i+1, " completed with ", len(times), " time points.")
	x1_trace.append(states[:,1,0])

x1_mean = np.mean(x1_trace, axis=0)
x1_std = np.std(x1_trace, axis=0)

print("Number of means:", len(x1_mean))

plt.plot(times, x1_mean, label='sample mean')
plt.plot(times, x1_mean - x1_std, '--', color='red', alpha=0.2, label='sample mean ± std')
plt.plot(times, x1_mean + x1_std, '--', color='red', alpha=0.2)
plt.legend()
plt.xlabel('Time')
plt.ylabel('X1')
plt.savefig('Ex3b_mean_std_plot.png')
