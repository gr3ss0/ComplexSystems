...
# 	##a) save trajectory
# 	Output = np.concatenate((np.array(times,ndmin=2),np.array(states[:,0,0],ndmin=2), np.array(states[:,1,0],ndmin=2)), axis=0)
# 	np.savetxt('Task3ATraj'+str(i+1)+'.txt',Output,delimiter = ',',fmt='%1.2f');	
...
for i in range(NrSimulations):
	...
	##a) save trajectory
	Output = np.concatenate((np.array(times,ndmin=2),np.array(states[:,0,0],ndmin=2), np.array(states[:,1,0],ndmin=2)), axis=0)
	np.savetxt('Task3ATraj'+str(i+1)+'Timed.txt',Output,delimiter = ',',fmt='%1.2f');
